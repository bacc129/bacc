	...
	if [ -d "${HOME}/bacc/opt/boost_1_67_0" ]; then
		if ! mv "${HOME}/bacc/opt/boost_1_67_0" "$BOOST_ROOT"
		then
			printf "\\n\\tUnable to move directory %s/opt/boost_1_67_0 to %s.\\n" "${HOME}/bacc" "${BOOST_ROOT}"
			printf "\\n\\tExiting now.\\n"
			exit 1
		fi
		if [ -d "$BUILD_DIR" ]; then
...
net:
 ...
 ipv6: true
storage:
 dbPath: ${HOME}/bacc/opt/mongodb/data
mongodconf
		then
	...
	if [ ! -d "${HOME}/bacc/opt/wasm/bin" ]; then
		...
		if ! cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="${HOME}/bacc/opt/wasm" -DLLVM_TARGETS_TO_BUILD= \
