...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if(    (create.creator != config::system_account_name)
      ...
      EOS_ASSERT( name_str.find( "noc." ) != 0 
               && name_str.find( "rank." ) != 0
               && name_str.find( "bacc." ) != 0
               && name_str.find( "sys." ) != 0, 
               action_validate_exception,
               "only privileged accounts can have names that start with keep word" );
   }
...
void apply_system_native_setcode(apply_context& context) {
   ...
   const auto is_open_set_code = is_func_has_open(context.control, config::func_typ::is_open_set_code);
   auto pay_account = act.account;
   if( !is_open_set_code ){
      pay_account = config::system_account_name;
   }
   ...
   if (new_size != old_size) {
      context.add_ram_usage( pay_account, new_size - old_size );
   }
}
...
void apply_system_native_setabi(apply_context& context) {
   ...
   context.require_authorization(pay_account);
...
void apply_system_native_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "noc." ) != 0, action_validate_exception,
               "Permission names that start with 'noc.' are reserved" );
   EOS_ASSERT( update.permission.to_string().find( "bacc." ) != 0, action_validate_exception,
               "Permission names that start with 'bacc.' are reserved" );
