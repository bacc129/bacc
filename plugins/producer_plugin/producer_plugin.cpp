   ...
   //                  ilog( "${n} confirmed", ("n",name(producer)) );
      ...
      void on_incoming_block(const signed_block_ptr& block) {
         ...
         if( fc::time_point::now() - block->timestamp < fc::minutes(5) || (block->block_num() % 1000 == 0) ) {
            const auto block_id_str = fc::variant(block->id()).as_string();
            ilog("Received:#${n}/${lib},${id_begin}..${id_tail},${t}[${latency} ms],${p}[${confs}],${count}",
                 ("p",block->producer)
                 ("id_begin",block_id_str.substr(8, 8))
                 ("id_tail",block_id_str.substr(block_id_str.size() - 8, 8))
...
void producer_plugin_impl::produce_block() {
   ...
   const auto new_b_id_str = fc::variant(new_bs->id).as_string();
   ilog("Produced:#${n}/${lib},${id_begin}..${id_tail},${t},${p}[${confs}],${count}",
        ("p",new_bs->header.producer)
        ("id_begin",new_b_id_str.substr(8, 8))
        ("id_tail",new_b_id_str.substr(new_b_id_str.size() - 8, 8))
        ("n",new_bs->block_num)
