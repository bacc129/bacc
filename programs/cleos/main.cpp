...
string default_wallet_url = "unix://" + (determine_home_directory() / "bacc-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
fc::variant call( const std::string& url,
   ...
   try {
      ...
      return eosio::client::http::do_http_call(*sp, fc::variant(v), true, true );
   }
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active, const name& vid) {
   return action {
...
chain::action create_setabi(const name& account, const bytes& abi) {
   return action {
      get_account_permissions(tx_permission, {config::system_account_name,config::active_name}),
      setabi{
         .account   = account,
         .abi       = abi
      }
...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "BACC") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
...
struct create_account_subcommand {
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      createAccount->add_option("Vid", vid, localized("Vid for new account"));
      ...
      createAccount->set_callback([this] {
            ...
            auto create = create_newaccount(creator, account_name, owner_key, active_key, vid);
            if (!simple) {
